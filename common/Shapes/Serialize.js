...
//                    uni_fill.fill.color.Mods = new AscFormat.CColorModifiers();
        ...
        while (s.cur < _end_rec)
        {
            ...
            switch (_at)
            {
                ...
                case 2:
                {
                    s.Skip2(4);
                    var _c = s.GetULong();
                    txbody.setContent(new AscCommon.CDocumentContent(txbody, this.presentation ? this.presentation.DrawingDocument : null, 0, 0, 0, 0, 0, 0, true));
                    if(_c>0)
                    {
                    ...
                    if(shape && (this.TempMainObject && typeof Slide !== "undefined" && this.TempMainObject instanceof  Slide && shape.isPlaceholder && shape.isPlaceholder() && (shape.getPlaceholderType() === AscFormat.phType_sldNum || shape.getPlaceholderType() === AscFormat.phType_dt)) && _last_field_type)
                    {
                        ...
                        {
                            txbody.content.Internal_Content_RemoveAll();
                            txbody.content.Internal_Content_Add(txbody.content.Content.length,  new AscCommon.Paragraph(txbody.content.DrawingDocument, txbody.content, 0, 0, 0, 0, 0, true));
        ...
        while (s.cur < _end_rec)
        {
            ...
            switch (_at)
            {
                ...
                case 2:
                {
                    ...
                    if(!txbody.content)
                        txbody.content = new AscCommon.CDocumentContent(shape, this.presentation ? this.presentation.DrawingDocument : null, 0, 0, 0, 0, 0, 0, true);
                    if(_c>0)
                    {
                        txbody.content.Internal_Content_RemoveAll();
                    }
    ...
    {
        var par = new AscCommon.Paragraph(DocumentContent.DrawingDocument, DocumentContent, 0, 0, 0, 0, 0, true);
            ...
            while (s.cur < _end_rec)
            {
                ...
                switch (_at)
                {
                    ...
                    case 4:
                    {
                        ...
                        shape.setTextBoxContent(new AscCommon.CDocumentContent(shape, this.LogicDocument.DrawingDocument, 0, 0, 0, 0, false, false));
